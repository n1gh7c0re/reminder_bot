...
- `reminder_name` сохраняет введенный текст и переходит к следующему шагу — вводу даты, обеспечивая возможность отмены через кнопку "cancel".
- `reminder_date` использует библиотеку `dateparser` для преобразования строки в объект даты. Если дата не распознана, пользователь получает сообщение "I don't understand" и может попробовать снова.
...
- **UML**: Переход `InputText → ChooseDate` реализован через `reminder_name` и `reminder_date`, что соответствует диаграмме состояний.
